...
Description : In this experiment, We compare a 3 layer Neural Network trained with backpropagation vs it's weights being trained through a couple of EML algorithms such as  Genetic Algorithms, Evolution Strategies and 
Particle Swarm Optimization. This experiment is used to understand if multilayer networks are trained athough EML algorithms provide better, equal or lesser performance. 
=#


#Functions to be used in the assignment
...
function make_model(mat)
	weight1 = mat[1:800]
	weight2 = mat[801:1600]
	weight3 = mat[1601:2400]
	weight4 = mat[2401:3200]
	weight5 = mat[3201:3240]
	weight1 = reshape(weight1,(40,20))
	weight2 = reshape(weight2,(20,40))
	weight3 = reshape(weight3,(40,20))
	weight4 = reshape(weight4,(20,40))
	weight5 = reshape(weight5,(2,20))
	weight1=convert(Array{Float32}, weight1)
	weight2=convert(Array{Float32}, weight2)
	weight3=convert(Array{Float32}, weight3)
	weight4=convert(Array{Float32}, weight4)
	weight5=convert(Array{Float32}, weight5)
	layer1 = Dense(weight1,false,x->σ.(x))
	layer2 = Dense(weight2,false,x->σ.(x))
	layer3 = Dense(weight3,false,x->σ.(x))
	layer4 = Dense(weight4,false,x->σ.(x))
	layer5 = Dense(weight5,false,x->σ.(x))
	model = Chain(layer1,layer2,layer3,layer4,layer5)
	return model
end
...
#We First calculate the accuracies of a neural network with 4 hidden layers
...
for i in 1:10
	...
	hidden_size1 = size(trn_features,1) * 2
	hidden_size2 = hidden_size1*2
	hidden_size3 = hidden_size2 *2
	layer_1 = Dense(size(trn_features,1),hidden_size1,x->σ.(x))
	hdlayer_1 = Dense(hidden_size1,size(trn_features,1),x->σ.(x))
	hdlayer_2 = Dense(size(trn_features,1),hidden_size1,x->σ.(x))
	hdlayer_3 = Dense(hidden_size1,size(trn_features,1),x->σ.(x))
	layer_2 = Dense(size(trn_features,1),size(trn_labels,1),x->σ.(x))
	model_nn = Chain(layer_1,hdlayer_1,hdlayer_2,hdlayer_3,layer_2)
	new_weight_mat = vcat(vec(layer_1.weight),vec(hdlayer_1.weight),vec(hdlayer_2.weight),vec(hdlayer_3.weight),vec(layer_2.weight))
	optimizer = Descent(0.01)
    iterator = Iterators.repeated((trn_features, trn_labels), 200);
    loss(x, y) = Flux.crossentropy(model_nn(x), y)
    Flux.train!(loss, Flux.params(model_nn),iterator, optimizer)
    acc_train = accuracy(model_nn,trn_features,trn_labels)
	conf_train = confusion_matrix(model_nn,trn_features,trn_labels)
    push!(total_accuracy_train,acc_train)
    acc_test=accuracy(model_nn,tst_features,tst_features)
    push!(total_accuracy_test,acc_test)
	conf_test = confusion_matrix(model_nn,tst_features,tst_labels)
	print_confusion(conf_train)
...
#A copy of the neural netwrok is provided here to see the design of the neural network
...
begin
	total_accuracy_train2 = []
	total_accuracy_test2 = []
	iterations2 = 1:10
end


#Using ES genetic algorithm to calulate the accuracies of the neural network after optmising it using evolution strategies 
...
for i in 1:10
    ...
    ES(initStrategy = IsotropicStrategy(N),recombination = average, srecombination 
    = average,mutation = cauchy, smutation = gaussian,selection=:plus,μ = 2, λ = 35
	),Evolutionary.Options(show_trace     =false ,iterations=100))
	#acc_train = 1/optimisedvalues.minimum
	#push!(total_accuracy_train2,acc_train)
	optimised_weights=optimisedvalues.minimizer
	acc_test=accuracy(model_nn,tst_features,tst_labels)
    push!(total_accuracy_test2,acc_test)
	modelnn=make_model(optimised_weights)
    ...
    #of the GA
	print_confusion(conf_test)
	println("Test accuracy of classifier :",((conf_test[1,1]+conf_test[2,2])/(conf_test[1,2]+conf_test[2,1] +conf_test[1,1]+conf_test[2,2])))
	acc_train =((conf_train[1,1]+conf_train[2,2])/(conf_train[1,2]+conf_train[2,1] +conf_train[1,1]+conf_train[2,2]))
    push!(total_accuracy_train2,acc_train)
	println("Iteration number is complete :",i)
...
length(total_accuracy_test2)


length(total_accuracy_train2)


iterations1 = log.(iterations)


plot(layer(x=iterations1,y=total_accuracy_train,Geom.point,Geom.line,color=[colorant"green"]),layer(x=iterations1,y=total_accuracy_test,Geom.point,Geom.line,color=[colorant"blue"]),layer(x=iterations1,y=total_accuracy_train2,Geom.point,Geom.line,color=[colorant"hotpink"]),layer(x=iterations1,y=total_accuracy_test2,Geom.point,Geom.line,color=[colorant"gold"]),Guide.Title("Comparision of Accuracy of 3HDNN vs ES"),Guide.xlabel("Log of iterations"),Guide.ylabel("Accuracy"))


#=
Intrepation of the results
*The neural network failed to get a even a decent accuracy for classyfing data
just bouncing around the 20 percent mark.
*This could be because the values were overfitted to the training set. But as the training set also doesnt have a high accuracy, the possibilty of overfitting is pretty low
*The Es on the other hand performed pretty well with around 65percent accuracy which is the highest I have gotten assignments.
*This could be because the method of how new individuals are created ES using mutation could be the reason for its higher performance.
=#
