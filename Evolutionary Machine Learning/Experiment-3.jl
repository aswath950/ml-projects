...
Description: In this ML and Evolutionary ML Expirement. We create a neural network which would act the medium to test our Evolutionary strategies algorithm, Evolution strategies is a commonly used EML algorithm along with
the genetic algorithm. We used it to compare the two algorithms and how they differ in training the neural network. Evolutionary Strategies has a better accuracy in training theNN as it more suited to continous values.
...
#=
A Loop which runs 10 times to create a Neural network with an Evolution strategy algorithm to classify mushrooms based on their attributes as edible or non-edible.

It uses the same random seeds as Assignment -1 and Assignment -2, therefore creating the same initial weights for the neural network as Assignment - 1 and Assignment -2 which helps us set of values that are levelled. 

We provide the flattened version of the nn weights to the ES algorithm for it to modify, We also provide the ES with a function to pass these weights to return the inverse of the accuracy , so that it minimises the value which would result in the max accuracy possible.
=#
...
for i in 1:10
	...
	),Evolutionary.Options(show_trace     =false ,iterations=25))
	...
	acc_train =((conf_test[1,1]+conf_test[2,2])/(conf_test[1,2]+conf_test[2,1] +conf_test[1,1]+conf_test[2,2]))
...
#=
We create a vector with accuracies from the previous assignments
We use these values to compare the accuracies with ES
We plot these accuracies to visualise them 
=#

# ╔═╡ 7b02292c-b984-40bf-a706-7dc393c908e9
GA_total_accuracy_train =[0.510143
0.506857
0.503286
0.507429
0.51
...
# ╔═╡ 705d5492-5f42-4219-ab65-6fb173536edf
GA_total_accuracy_test =[0.485333
0.498333
0.492333
0.501667
0.500333
...
# ╔═╡ b4bf3b8b-83d3-4a7c-b161-e244655d9e0f
total_accuracy_train

# ╔═╡ 64c1b6b3-8dbf-4148-a990-c3fe44512e2b
total_accuracy_test
...
# ╔═╡ 4958d737-661c-4810-b35e-d1e5085bc78b
length(total_accuracy_test)

# ╔═╡ 6538c46d-923a-41ff-95cb-7160ab3db3d1
length(total_accuracy_train)

# ╔═╡ 320a3fde-9133-4f4b-82de-134e4f8e5ade
#We plot the first graph of ES vs Neural networks to see how they relate
...
# ╔═╡ 1370c3ba-0954-4cce-8360-aa4df40ca359
#The second graph is ES vs GA which was created in HW-2

# ╔═╡ 2bd97bc4-f0a6-45cd-8eb8-720f106d4469
plot(layer(x=iterations1,y=total_accuracy_train,Geom.point,Geom.line,color=[colorant"green"]),layer(x=iterations1,y=total_accuracy_test,Geom.point,Geom.line,color=[colorant"blue"]),layer(x=iterations1,y=GA_total_accuracy_train,Geom.point,Geom.line,color=[colorant"hotpink"]),layer(x=iterations1,y=GA_total_accuracy_test,Geom.point,Geom.line,color=[colorant"gold"]),Guide.Title("Comparision of Accuracy"),Guide.xlabel("Log of iterations"),Guide.ylabel("Accuracy"))

# ╔═╡ db2d2423-2194-4cb8-a47e-e3905bd0ae59
#=
Inference of plots and accuracy of Evolutionary Strategies 
*As seen from the above graphs. We can see that ES has better accuracy compared to that of both NN and GA.
*This could be because ES are better suited for continuous values to that of GAs. With ES being able to self-adapt its parameters  based on its progress in the right or wrong direction.
*The accuracy could possibly be improved more if tweaks are done to the parameters of the algorithm.
