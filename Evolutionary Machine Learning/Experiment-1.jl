...
Expirement 1  : Preprocessing a dataset, Using it to train a simple 1 layered Neural Network(NN). The model created will be used as a source of truth in future expirements 
=#
...
#= 
The dataset holds values of different features  of mushrooms which  are classified into either class = "p" which means poisonous and class = "edible"
meaning edible. 
The unprocessed dataset contains 21 attributes with 'class' being one of it.
The dataset can be acquired from the link : https://archive.ics.uci.edu/ml/datasets/Secondary+Mushroom+Dataset

The dataset contains almost 60000+ rows. But for space complexity and time purposes, We randomly acquired 5000 class 'p' rows and 5000 class 'e' rows.
After  choosing certains attributes to be one-hot-encoded, we acquire a dataset of shape 10000 x 21 with equal count of classes. 
=# 


mushroom_features,mushroom_labels = get_features_and_labels(mushrooms)
...
#=
Now we run a loop 10 times , which does the following work
1. Based on the random weight acquires test and train data
2. Creates a shall Neural network with one hidden layer with the required specifications
3. Stores the weights of the layers of  neural networks in into weights_layer1.csv and weights_layer2.csv respectivly
4. Trains the model with the training data
5. calculates the accuracy and confusion matrix for that model on training and test data
=#
...
for i in 1:10
    ...
    layer_1 = Dense(size(trn_features,1), hidden_size,x->σ.(x))
    layer_2 = Dense(hidden_size, size(trn_labels,1),x->σ.(x))
    model_nn = Chain(layer_1,layer_2)
    CSV.write("MushroomDataset/weights_layer1.csv",  Tables.table(layer_1.weight), 
    writeheader=true,append = true)
    CSV.write("MushroomDataset/weights_layer2.csv",  Tables.table(layer_2.weight), 
    writeheader=true,append = true)
    optimizer = Descent(0.01)
    iterator = Iterators.repeated((trn_features, trn_labels), 1000);
    loss(x, y) = Flux.crossentropy(model_nn(x), y)
    Flux.train!(loss, params(model_nn),iterator, optimizer)
...
plot(iterations1,total_accuracy_train,title = "accuracy vs log of iterations",label="train data",lw = 3 )


plot!(iterations1,total_accuracy_test,title = "accuracy vs log of iterations",label="test data",lw = 3 )
